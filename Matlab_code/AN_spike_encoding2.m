function [ofp,onset,filtered] = AN_spike_encoding2(input,param)
...
%         aux{j} = LIFneuron(Cadj,leak,param);
%     end
%     onset{i} = aux;
% end
...
aux = sum(onset_fp);
activity = find(aux~=0,1,'first');
ofp = onset_fp(:,activity:activity+param.len_ofp-1);
